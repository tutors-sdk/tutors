...
export function calendar(session: Session, calendarMap: CalendarMap[], bgPatternImg: HTMLImageElement, currentRange: string): EChartsOption {
  return {
    ...
    series: {
      ...
      data: calendarMap.map((calendar) => [echarts.time.format(calendar.date, "{yyyy}-{MM}-{dd}", false), Math.round(calendar.timeActive / 2) || 0]) || []
    },
    label: {
      show: true,
...
export function calendarCombined(
  ...
  return {
    ...
    series: {
      type: "heatmap",
      coordinateSystem: "calendar",
      data: Array.from(calendarMap)?.map(([date, timeActive]) => [echarts.time.format(date, "{yyyy}-{MM}-{dd}", false), Math.round(timeActive / 2) || 0]) || []
    },
    label: {
      show: true,
      formatter: function (params: { value: any[] }) {
