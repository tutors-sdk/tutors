<script lang="ts">
  ...
  let { courseid, studentid, calendarByDay, medianRow, dates }: Props = $props();
  ...
  function formatCellTime(minutes: number | undefined): string {
    if (minutes == null || minutes === 0) return "—";
...
{#if calendarByDay}
  <section class="card p-6">
    <h2 class="text-2xl font-semibold mb-4">Calendar Activity by Day</h2>
      ...
      <table class="w-full border-collapse" style="table-layout: fixed;">
        ...
        <tbody>
          ...
          <tr class="border-b border-surface-200 hover:bg-surface-50">
            ...
            {#each dates as date}
              {@const dateMinutes = calendarByDay[date] as number | undefined}
              {@const mins = dateMinutes != null ? dateMinutes : 0}
              <td class="py-3 px-1 text-center font-mono text-xs" style="width: 36px; min-width: 36px; max-width: 36px; background-color: {cellColorForMinutes(mins)}">
                {formatCellTime(dateMinutes)}
              </td>
            {/each}
            <td class="py-3 px-4 text-right font-mono font-semibold" style="background-color: {cellColorForMinutes(calendarByDay.totalSeconds ?? 0)}">
              {formatTime(calendarByDay.totalSeconds)}
            </td>
          </tr>
          <!-- Median Row -->
          {#if medianRow}
            ...
            <tr class="border-b-2 border-surface-300 bg-surface-100">
              ...
              {#each dates as date}
                {@const dateBlocks = medianRow[date] as number | undefined}
                <td class="py-3 px-1 text-center font-mono text-xs" style="width: 36px; min-width: 36px; max-width: 36px; background-color: {cellColorForMinutes(dateBlocks ?? 0)}">
                  {formatCellTime(dateBlocks)}
                </td>
              {/each}
              <td class="py-3 px-4 text-right font-mono font-semibold" style="background-color: {cellColorForMinutes(totalBlocks)}">
                {formatCellTime(medianRow.totalSeconds)}
              </td>
            </tr>
          {/if}
        </tbody>
