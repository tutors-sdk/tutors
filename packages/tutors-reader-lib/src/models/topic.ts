...
export class Topic {
  ...
  constructor(lo: Lo, courseUrl: string, course: Course) {
    ...
    const standardLos = this.lo.los.filter((lo) => lo.type !== "unit" && lo.type !== "panelvideo" && lo.type !== "paneltalk" && lo.type !== "panelnote" && lo.type !== "side");

    this.toc.push(...panelVideos, ...panelTalks, ...panelNotes, ...units, ...standardLos, ...sideBar);
