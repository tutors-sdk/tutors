...
```javascript { .api }
const statement = doc.builders.group(["if (", "condition", ")", doc.builders.line, "doSomething();"]);
...
```javascript { .api }
const array = doc.builders.group(["[", doc.builders.softline, doc.builders.join([",", doc.builders.line], elements), doc.builders.softline, "]"]);
// Result: '[a, b, c]' or '[\n  a,\n  b,\n  c\n]'
```
...
const functionCall = doc.builders.group([
  "functionName(",
  doc.builders.indent([doc.builders.softline, doc.builders.join([",", doc.builders.line], args)]),
...
const alternatives = doc.builders.conditionalGroup([
  ...
  ["(", doc.builders.indent([doc.builders.line, doc.builders.join([",", doc.builders.line], args)]), doc.builders.line, ")"]
]);
```
...
const paragraph = doc.builders.fill(doc.builders.join(doc.builders.line, words));
...
```javascript { .api }
const block = ["{", doc.builders.indent([doc.builders.hardline, "statement1;", doc.builders.hardline, "statement2;"]), doc.builders.hardline, "}"];
// Result: '{\n  statement1;\n  statement2;\n}'
```
...
```javascript { .api }
const conditional = doc.builders.group(["if (", doc.builders.indentIfBreak([doc.builders.line, "longCondition"]), doc.builders.line, ")"]);
...
```javascript { .api }
const aligned = doc.builders.align(4, ["let x = longVariableName +", doc.builders.line, "anotherLongVariableName;"]);
...
const arrayElements = doc.builders.group([
  "[",
  doc.builders.indent([doc.builders.softline, doc.builders.join([",", doc.builders.ifBreak(doc.builders.hardline, doc.builders.line)], elements)]),
  doc.builders.softline,
  "]"
]);
```
...
const document = doc.builders.group([
  "function(",
  doc.builders.indent([doc.builders.softline, doc.builders.join([",", doc.builders.line], parameters)]),
  doc.builders.softline,
  ")"
]);
...
function formatObjectExpression(properties) {
  return doc.builders.group([
    "{",
    properties.length > 0 ? [doc.builders.indent([doc.builders.line, doc.builders.join([",", doc.builders.line], properties.map(formatProperty))]), doc.builders.line] : "",
    "}"
  ]);
}
...
function formatArray(elements, options) {
  const shouldBreakElements = elements.length > 3 || elements.some((el) => willBreak(el));
      ...
      elements.length > 0
        ? [
            doc.builders.indent([
              ...
              doc.builders.join([",", shouldBreakElements ? doc.builders.hardline : doc.builders.line], elements)
            ]),
            shouldBreakElements ? doc.builders.hardline : doc.builders.softline
          ]
        : "",
