...
const invokeWithRetry = async (functionName: string, options: FunctionInvokeOptions, maxRetries: number = 3, delay: number = 1000): Promise<FunctionResponse<any>> => {
...
// Usage
const { data, error } = await invokeWithRetry("unreliable-function", { body: { data: "test" } }, 3, 1000);
```
...
const executeBatch = async (
  ...
  const promises = functionCalls.map((call) => supabase.functions.invoke(call.name, call.options));
...
const EDGE_FUNCTION_URL = isDevelopment ? "http://localhost:54321/functions/v1" : undefined;
