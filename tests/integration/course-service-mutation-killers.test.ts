...
describe("Course Service - Mutation Killers", () => {
  ...
  describe("WHEN fetch fails with different status codes", () => {
    it("shall throw error with exact status 400", async () => {
      ...
      await expect(courseService.getOrLoadCourse("bad", mockFetch)).rejects.toThrow(/400/);
    });
    ...
    it("shall throw error with exact status 403", async () => {
      ...
      await expect(courseService.getOrLoadCourse("forbidden", mockFetch)).rejects.toThrow("Fetch failed with status 403");
    });
  });
  ...
  describe("WHEN JSON parsing fails", () => {
    it("shall propagate the exact error", async () => {
      ...
      await expect(courseService.getOrLoadCourse("malformed", mockFetch)).rejects.toThrow("Unexpected token");

      await expect(
        courseService.getOrLoadCourse(
          "malformed",
          mockFetch.mockResolvedValueOnce({
            ok: true,
            json: async () => {
  ...
  describe("WHEN network errors occur", () => {
    it("shall propagate network error unchanged", async () => {
      ...
      await expect(courseService.getOrLoadCourse("offline", mockFetch)).rejects.toBe(networkError);
    });
    ...
    it("shall propagate DNS errors unchanged", async () => {
      ...
      await expect(courseService.getOrLoadCourse("unknown", mockFetch)).rejects.toThrow("ENOTFOUND");
    });
  });
