<script lang="ts">
  ...
  if (
    ...
    student.topic.lo.frontMatter &&
    student.topic.lo.frontMatter.icon
  ) {
    student.topic.lo.icon = {
...
<div class="card h-90 border-primary-500 m-1 w-4/5 overflow-x-hidden border-y-8 lg:w-2/5">
  <div class="flex">
  ...
  <div class="card-body">
    {#if student.topic && !student.lab}
      <div class="my-2 justify-center">
        {#if student.topic.lo.icon}
          <Iconify
            icon="{student.topic.lo.icon.type}"
            color="{student.topic.lo.icon.color}"
            height="{iconHeight}"
          />
        {:else}
          <img
            loading="lazy"
            class="mx-auto {imageHeight}"
            src="{student?.topic?.lo.img}"
            alt="{student?.topic?.lo.title}"
          />
        {/if}
      </div>
    {/if}
    {#if student.lab && !student.topic}
      <div class="my-2 justify-center">
        {#if student.lab.icon}
          <Iconify
            icon="{student.lab.icon.type}"
            color="{student.lab.icon.color}"
            height="{iconHeight}"
          />
        {:else}
          <img
