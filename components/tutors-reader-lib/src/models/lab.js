...
export class Lab {
    ...
    content = "";
    chaptersHtml = new Map();
    chaptersTitles = new Map();
    steps = [];
    autoNumber = false;
    vertical = true;
    constructor(course, lo, url) {
        this.course = course;
        ...
        this.lo.los.forEach((chapter, i) => {
            ...
            if (encodeURI(chapter.shortTitle) == this.currentChapterShortTitle) {
                if (this.lo.los[i - 1] !== undefined) {
                    ...
                    let step = `${i - 1}:`;
                    this.horizontalNavbarHtml = this.horizontalNavbarHtml.concat(`<a class="btn btn-sm capitalize" href="/#/lab/${this.url}/${encodeURI(nav.shortTitle)}"> <span aria-hidden="true">&larr;</span>&nbsp; ${number}${title} </a>`);
                }
                if (this.lo.los[i + 1] !== undefined) {
                    let nav = this.lo.los[i + 1];
                    let title = this.chaptersTitles.get(nav.shortTitle);
                    let step = `${i + 1}:`;
                    this.horizontalNavbarHtml = this.horizontalNavbarHtml.concat(`<a class="ml-auto btn btn-sm capitalize" style="margin-left: auto" href="/#/lab/${this.url}/${encodeURI(nav.shortTitle)}"> ${number}${title} &nbsp;<span aria-hidden="true">&rarr;</span></a>`);
                }
            }
        });
        this.navbarHtml = nav;
